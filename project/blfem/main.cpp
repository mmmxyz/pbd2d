...
void projection()
{
		...
		for (uint32_t x = 0; x < 3; x++)
		{
				...
				for (uint32_t i = 0; i < 4 * N; i++)
				{
						...
						float e11 = F.m[0] - 1.0;
						float e22 = F.m[3] - 1.0;
						float e12 = 0.5 * (F.m[1] + F.m[2]);

						float T11 = lambda * (e11 + e22) + 2.0 * mu * e11;
						float T22 = lambda * (e11 + e22) + 2.0 * mu * e22;
						float T12 = 2.0 * mu * e12;
						...
						float W = T11 * e11 + T12 * e12 + T12 * e12 + T22 * e22;

						float vol = mat2(x1 - x0, x2 - x0).det();
						float C = std::sqrt(2.0 * vol * W);
...
int main(int argc, char const *argv[])
{
		...
		while (!mywindow.is_shouldclose())
		{
				...
				for (uint32_t i = 0; i < 3 * (N + 1); i++)
				{
						tempu[i] = u[i] + dt * v[i] + dt * dt * fvec2(0.0, -0.10);
				}
